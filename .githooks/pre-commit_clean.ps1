...
if ($errors.Count -gt 0) {
    Write-Output " Commit blocked: ScriptAnalyzer found errors." -ForegroundColor Red
    exit 1
}

Write-Output " ScriptAnalyzer passed. Proceeding with sensitive file check..."

# Sensitive file patterns
$blockedPatterns = @(
    'id_ed25519', '\.ssh', 'known_hosts', '\.pub$', '_site/'
)
...
foreach ($file in $stagedFiles) {
    foreach ($pattern in $blockedPatterns) {
        if ($file -match $pattern) {
            Write-Output " Commit blocked: sensitive file detected - | $file" -ForegroundColor Red
            $foundBlocked = $true
        }
    }
}
...
if ($largeFiles.Count -gt 0) {
    Write-Output " Warning: Large files staged ( | 5 MB):" -ForegroundColor Yellow
    $largeFiles | ForEach-Object { Write-Output "   $_" }
}
