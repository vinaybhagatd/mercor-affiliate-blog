    ...
    if (-not ($content -join "`n" -match '<#')) {
        for ($i = 0; $i -lt $content.Count; $i++) {
            if ($content[$i] -match '^\.(SYNOPSIS|DESCRIPTION)') {
                $commentStart = $i
                while (($commentStart > 0) -and ($content[($commentStart-1)] -notmatch '<#')) {
                    $commentStart--
                }
                if ($commentStart -lt 0) {
                    Write-Host "Skipping metadata header in $($script.Name) due to no comment block"
                    continue
                }

                # Remove the leading comment from the header and add it inside <# ... #> blocks
                $newComment = $content[$($commentStart+1)..$i].Join('')
                $content[$commentStart..$i] = @("<#", $newComment, "#>") | ForEach-Object { $_.Trim() }
                $fixed = $true
