...
foreach ($script in $scripts) {
    ...
    # 5. Add missing closing brace at EOF
    if ($content.Count -gt 0 -and $content[-1] -notmatch '^\}$') {
        $content += "}"
        $fixed = $true
    ...
    if ($fixed) {
        ...
        Set-Content -Path $script.FullName -Value $sanitized -NoNewline -Encoding UTF8

        try {
            Invoke-Formatter -ScriptDefinition (Get-Content $script.FullName -Raw) |
                Set-Content -Path $script.FullName -Encoding UTF8
