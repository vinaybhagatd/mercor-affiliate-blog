...
Write-Host "=== SelfHeal.ps1 started ===" -ForegroundColor Cyan
...
foreach ($file in $files) {
    ...
    if (-not ($content -match "category:\s*(\w+)")) {
        ...
        Write-Host "[SelfHeal] Inserted missing category [$newCategory] into $($file.Name)" -ForegroundColor Yellow
    ...
    if (-not ($content -match "\(https:\/\/t\.mercor\.com\/[A-Za-z0-9]+\)")) {
        if ($cat -and $affiliateLinks.ContainsKey($cat)) {
            ...
            Write-Host "[SelfHeal] Inserted missing affiliate link [$link] into $($file.Name)" -ForegroundColor Yellow
